...
import type { EvangelizationColor, MemberRole, Profile } from "@/types/database";
...
interface AuthContextValue {
  ...
  signUp: (params: {
    ...
    city: string;
    state?: string;
    evangelizationColor: EvangelizationColor;
...
const STAFF_ROLES: MemberRole[] = ["staff", "admin"];
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(
    () => ({
      ...
      async signUp({ email, password, fullName, city, state, evangelizationColor, membershipLevelId }) {
        ...
        if (data.user) {
          const { error: profileError } = await supabase.from("profiles").insert({
            ...
            city,
            state: state ?? null,
            evangelization_color: evangelizationColor,
